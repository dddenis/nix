...
import {
  ...
  buildStatsLineEffect,
  buildUsageStatsPartsEffect,
  formatInlineOpenAIStatusEffect,
  summarizeAssistantUsageEffect,
...
describe("custom footer helpers", () => {
  test("formats context as remaining percentage with inverted severity", () => {
    expect(
      formatRemainingContextDisplay(
        { percent: 18.24, contextWindow: 200_000 },
        200_000,
        true,
      ),
    ...
    expect(
      formatRemainingContextDisplay(
        { percent: 75, contextWindow: 200_000 },
        200_000,
        true,
      ),
    ).toEqual({
    ...
    expect(
      formatRemainingContextDisplay(
        { percent: 95, contextWindow: 200_000 },
        200_000,
        false,
      ),
    ).toEqual({
    ...
    expect(
      formatRemainingContextDisplay(
        { percent: null, contextWindow: 200_000 },
        200_000,
        true,
      ),
    ).toEqual({
  ...
  it.effect("builds footer stats through Effect APIs", () =>
    Effect.gen(function* () {
      const summary = yield* summarizeAssistantUsageEffect([
        {
          type: "message",
          message: {
            role: "assistant",
      ...
      expect(summary.input).toBe(1_200);
      expect(
        yield* formatInlineOpenAIStatusEffect("OpenAI limits loading"),
      ).toEqual(Option.some("| loading"));
      expect(
        yield* buildStatsLineEffect({
          width: 80,
          statsParts: yield* buildUsageStatsPartsEffect(summary, true),
  ...
  test("formats OpenAI status for inline footer display with a separator", () => {
    expect(formatInlineOpenAIStatus("OpenAI 5h 99% ↺2h | wk 92% ↺5d")).toBe(
      "| 5h 99% ↺2h | wk 92% ↺5d",
    );
    expect(formatInlineOpenAIStatus("OpenAI limits loading")).toBe("| loading");
    expect(formatInlineOpenAIStatus("OpenAI limits unavailable")).toBe(
      "| unavailable",
    );
