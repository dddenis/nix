...
function scoreFuzzySync(query: string, text: string): number | null {
...
export function scoreFuzzyEffect(
  query: string,
  text: string,
): Effect.Effect<Option.Option<number>> {
  return Effect.sync(() => Option.fromNullable(scoreFuzzySync(query, text)));
}

export function scoreFuzzy(query: string, text: string): number | null {
  return Option.getOrNull(runSyncEffect(scoreFuzzyEffect(query, text)));
}
...
function dedupeHistoryItemsSync(items: readonly HistoryItem[]): HistoryItem[] {
...
export function dedupeHistoryItemsEffect(
  items: readonly HistoryItem[],
): Effect.Effect<HistoryItem[]> {
  return Effect.sync(() => dedupeHistoryItemsSync(items));
}

export function dedupeHistoryItems(
  items: readonly HistoryItem[],
): HistoryItem[] {
  return runSyncEffect(dedupeHistoryItemsEffect(items));
}

export function searchHistoryItemsEffect(
  items: readonly HistoryItem[],
  query: string,
  scope: HistoryScope,
  currentCwd: string,
): Effect.Effect<HistorySearchResult[]> {
  return Effect.gen(function* () {
    const scoped = filterByScope(items, scope, currentCwd);
    const deduped = yield* dedupeHistoryItemsEffect(scoped);
    ...
    for (const item of deduped) {
      const score = yield* scoreFuzzyEffect(trimmedQuery, item.text);
      if (Option.isSome(score)) results.push({ item, score: score.value });
    }
...
export function searchHistoryItems(
  ...
  return runSyncEffect(
    searchHistoryItemsEffect(items, query, scope, currentCwd),
  );
}
