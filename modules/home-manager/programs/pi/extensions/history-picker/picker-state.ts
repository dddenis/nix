...
import { searchHistoryItemsEffect } from "./history-search";
...
export class HistoryPickerState {
  ...
  moveSelection(delta: number): void {
    ...
    this.selectedIndex = Math.max(
      0,
      Math.min(results.length - 1, this.selectedIndex + delta),
    );
  }

  getResultsEffect(): Effect.Effect<HistorySearchResult[]> {
    return searchHistoryItemsEffect(
      [...this.currentItems, ...this.savedItems],
      this.query,
      this.scope,
      this.currentCwd,
    );
  ...
  getResults(): HistorySearchResult[] {
    return runSyncEffect(this.getResultsEffect());
  }

  getSelectedItemEffect(): Effect.Effect<HistoryItem | undefined> {
    return this.getResultsEffect().pipe(
      Effect.map((results) => results[this.selectedIndex]?.item),
    );
  }

  getSelectedItem(): HistoryItem | undefined {
    return runSyncEffect(this.getSelectedItemEffect());
  }
  ...
  private clampSelection(): void {
    ...
    this.selectedIndex = Math.max(
      0,
      Math.min(this.selectedIndex, results.length - 1),
    );
  }
}
